...
# DOCKER_COMPOSE_INSTALL = "curl -L https://github.com/docker/compose/releases/download/1.29.2/docker-compose-$(uname -s)-$(uname -m) -o /usr/local/bin/docker-compose"
DOCKER_COMPOSE_INSTALL = "curl -L https://github.com/docker/compose/releases/download/1.29.2/docker-compose-{u_s}-{u_m} -o /usr/local/bin/docker-compose"
DOCKER_COMPOSE_INSTALL2 = "chmod +x /usr/local/bin/docker-compose"
LUNCHBOX_INSTALL_GIT = "git clone https://github.com/sy5surf/RTLunchBox.git"
LUNCHBOX_INSTALL_SCP = "scp -r ../lunchbox {target}:~/"
SSH_OR = " || echo \"error\""
GET_IP = "curl -4 -s icanhazip.com"
GET_LOCAL_IP = "ifconfig enp2s0 | cut -f 10 -d " " | head -n 2" #Need to test/try more reliable way to get local IP
GET_USER_IP = "" ## ARGS
...
def print_banner(arg = ""):
    ...
    banner_top = '''
 ________ ____ ____ ____ ____ ____ ____ ____ ____ ____________ 
|    ||R |||T |||L |||u |||n |||c |||h |||B |||o |||x ||      |
...
|_____________________________________________________________|
    ...
    if len(arg) != 0:
        ...
        print("\n\n\t\tEnjoy" + c.fg.red+ " LunchBox!" + c.bold + " <3" +c.reset)
...
def deploy_local():
    ...
    # Start deploy
    c.good_news(c, "Deploying lunchbox")
    output = run_cmd_output(DOCKER_DEPLOY)
    print(output)
    c.good_news(c, "Done")
    ip = run_cmd_output(GET_IP)
    ipint = run_cmd_output(GET_LOCAL_IP)
    print(c.bold + c.fg.green + "\n" + "=========================================================================" + c.reset)
    c.good_news(c, "Please find your running instance at [external] https://" + ip +"/portainer")
    c.good_news(c, "Please find your running instance at [internal] https://" + ipint +"/portainer")
    c.info_news(c, "Files are available at [external] https://" + ip + "/files")
    c.info_news(c, "Files are available at [internal] https://" + ipint + "/files")
    c.info_news(c, "Live Reverse Proxy data is available at [external] https://" + ip + "/api")
    c.info_news(c, "Live Reverse Proxy data is available at [internal] https://" + ipint + "/api")
...
def deploy_remote_ssh():
    '''
    Deploy lunchbox to a remote server using SSH
    '''
    ...
    if output != "error" and len(output) != 0:
        ...
        c.info_news(c, "Cloning latest lunchbox repository")
        run_cmd_output(SSH_CMD + LUNCHBOX_INSTALL_GIT + SSH_OR)
    elif output == "error":
        c.bad_news(c, "git installation not found")
        c.info_news(c, "Sending local files...")
        run_cmd_output(LUNCHBOX_INSTALL_SCP.format(target = username+"@"+target))
    ...
    # Get lunchbox; Deploy
    c.good_news(c, "Deploying lunchbox")
    run_cmd_output(SSH_CMD + "cd lunchbox ; " + DOCKER_DEPLOY + SSH_OR)
    c.good_news(c, "Done")
...
def deploy_dockermachine():
    ...
    if out != "Running":
        ...
        c.good_news(c, "Deploying lunchbox remotely using docker-machine " + c.bold + machine_name + c.reset)
...
def menu_deploy_target():
    ...
    while True:
        ...
        c.menu_item(c, "1", "Deploy lunchbox on local machine/server")
        c.menu_item(c, "2", "Deploy lunchbox on remote ssh machine")
        c.menu_item(c, "3", "Deploy lunchbox on remote docker-machine")
        c.menu_item(c, "4", "Stop local or docker-machine lunchbox deployment")
        c.menu_item(c, "5", "Stop remote ssh lunchbox deployment")
        ...
        choice = input(">> ")
        if choice == "1":
            c.good_news(c, "Deploying lunchbox locally")
            deploy_local()
        elif choice == "2":
            deploy_remote_ssh()
        elif choice == "3":
            ...
            if stop_out is not None:
                c.good_news(c, "Stopped running lunchbox stack")
            else:
            ...
            if username == "":
                username = "root"
            SSH_CMD = "ssh " + username+"@"+target+" "
            stop_out = run_cmd_output(SSH_CMD + "cd lunchbox;" + DOCKER_DOWN)
            # run_cmd_output(SSH_CMD + "cd lunchbox;" + "docker-compose kill")
            if stop_out is not None:
                c.good_news(c, "Stopped running ssh lunchbox stack")
            else:
                c.bad_news(c, "App Templates need to be stopped manually. Use 'docker-compose kill' to force Portainer down")
            input("\n- Press Enter to continue -")
...
if __name__ == "__main__":
    ...
    if "bash" not in os.getenv("SHELL"):
        c.info_news(c, "protip: lunchbox works best using bash or sh")
    if not os.geteuid()==0:
